...
class ExperimentSet(object):
    ...
    expected_params = set(['perc_null', 'num_study_genes', 'num_experiments', 'num_goeasims'])

    def __init__(self, params, tic, objbg):
        self.params = params
        self.objbg = objbg
    ...
    def get_strhdr(self):
        ...
        return "ExperimentSet({N}) {EXP}".format(N=self.params['num_experiments'], EXP=self.get_desc())

    def prt_num_goeasims_w_errs(self, prt=sys.stdout):
        """Print if errors were seen in sims."""
        desc = self.get_desc()
        prt.write("\n") # Separate sets of experiments
        for experiment in self.expset:
            experiment.prt_num_goeasims_w_errs(prt, desc)

    def _init_experiments(self, tic):
        """Run a set of experiments."""
        expset = []
        sys.stdout.write("{DESC} HMS={HMS}\n".format(DESC=self.get_strhdr(), HMS=get_hms(tic)))
        shared_param_keys = ['num_goeasims', 'num_study_genes', 'perc_null']
        for _ in range(self.params['num_experiments']):
            ...
            expset.append(ManyGoeaSims(experiment_params))
        return expset
